...
**Core dependencies** (`requirements.txt`): `openai`, `python-dotenv`,
`pydantic`, `colorlog`, `tenacity`.

**Additional dependencies for full functionality:**

| Package | Needed for | If missing |
|---|---|---|
| `jsonschema` | `SpecValidatorAgent` (schema validation + auto-repair) | Validation is skipped with a logged error; the enriched spec is still written. |
| `json_repair` | Optional JSON-recovery strategy for truncated LLM responses | That single recovery strategy is skipped; the other strategies still run. |
